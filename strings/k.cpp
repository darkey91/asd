...
public:
    ...
    void lcp(std::vector<int> &lcp, const std::vector<long long> &h, const std::vector<long long> &d) {
        for (int i = 1; i < str.size(); ++i) {
            lcp.push_back(bin_search(str, std::min(suf_array[i - 1], suf_array[i]), std::max(suf_array[i - 1], suf_array[i]), h, d));
        }
    }
